...
import { HobbyReport } from '../../Component/Services/HobbyReport/index.js';
const HobbyCheck = ({ userId }) => {
  ...
  const UserApi = async () => {
    setLoading(true);
    try {
      const response = await HobbyReport({ profile_id: userId });
        ...
        <div onClick={handleClose} className="fixed bg-black bg-opacity-50 z-50 h-[100vh] left-0 w-[100%] inset-0 py-20">
          <div onClick={(e) => e.stopPropagation()} className="h-[100%] mx-auto w-[70%]">
            ...
            <div className='absolute right-2 top-2 cursor-pointer' onClick={handleClose}><RxCross1 style={{fontSize:"15px"}}/></div>
              <div className="font-semibold text-3xl mt-5 text-center">User's Hobby</div>
              <div className="text-center">Brief outline of User's Hobbies</div>
